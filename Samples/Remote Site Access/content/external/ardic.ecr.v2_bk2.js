...
var utils = function () {
    ...
    return {
        ...
        setAppNameVisible: function(a){
           if(typeof a != 'undefined') {n = a; chrome.storage.local.set({'appName': a});} 
           if(n){$('.thumbnail p').show();}else{$('.thumbnail p').hide();}
        },
        setTimerTick: function(a){
            if(typeof a != 'undefined') { TimerTick = a; chrome.storage.local.set({'timerTick': a}); this.clearTimer();}
            this.setTimer();
        },
        setTimer: function(){
             var Timer4Json = setInterval(function(){ initMarquee(); initSettings(); initApp(); initMarket(); },TimerTick);
             var Timer4Wireless = setInterval(function(){checkWireless()},500000);
             var Timer4Bluetooth = setInterval(function(){checkBluetooth()},500000);
             var Timer4USB = setInterval(function(){checkCloud()},500000);
        },
        clearTimer: function() {
        ...
        setServicesURL: function (a) {
            s = a;
            chrome.storage.local.set({'ServicesURL':a});
            initConfig();
        },
...
function setTheme (a) {
    ...
    chrome.storage.local.set({'theme': a});
    switch(a){
...
function initReady() {
    ...
    // Load Local Settings
    chrome.storage.local.get("ServicesURL", function(data) {if(typeof data.ServicesURL != 'undefined') {utils.setServicesURL(data.ServicesURL); }else{utils.setServicesURL(s);} $('#Info #serviceip').html(s); utils.setInfo();});
    chrome.storage.local.get("appName", function(data) {utils.setAppNameVisible(data.appName);});
    chrome.storage.local.get("theme", function(data) {setTheme(data.theme);});
    chrome.storage.local.get("WirelessStatus", function(data) {if(typeof data.WirelessStatus != 'undefined') {setWireless(data.WirelessStatus);}else{ setWireless(window.navigator.onLine) ;}});
    chrome.storage.local.get("BluetoothStatus", function(data) {if(typeof data.BluetoothStatus != 'undefined') {setBluetooth(data.BluetoothStatus);}});
    chrome.storage.local.get("CloudStatus", function(data) {if(typeof data.CloudStatus != 'undefined') {setCloud(data.CloudStatus);}else{ setCloud(window.navigator.onLine); }});
...
function setWireless (a){
    ...
    chrome.storage.local.set({'WirelessStatus': a});
    var status = a;
    if(status)
    {
...
function setBluetooth (a){
    ...
    chrome.storage.local.set({'BluetoothStatus': a});
    var status = a;
    if(status)
    {
...
function setCloud(a){
    ...
    chrome.storage.local.set({'CloudStatus': a});
